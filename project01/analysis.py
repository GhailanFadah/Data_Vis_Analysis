...
class Analysis:
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        fig, axis = plt.subplots(len(data_vars), len(data_vars), sharex=True, sharey=True, figsize= fig_sz)

      
        for i in range(len(data_vars)):
            for x in range(len(data_vars)):
                
                axis[i, x].scatter(self.data.select_data([data_vars[i]]), self.data.select_data([data_vars[x]]))
                ...
                if i == len(data_vars) - 1:
                    axis[i, x].set_xlabel(data_vars[x])
